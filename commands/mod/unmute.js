...
module.exports = {
    ...
    run: async (bot, message, args) => {
        if (!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send("**Tu n'as pas la permission d'executer cette commande !**");

        if (!message.guild.me.hasPermission("MANAGE_GUILD")) return message.channel.send("**I Don't Have Permissions To Unmute Someone!**")
        if (!args[0]) return message.channel.send("**Tu as oublié l'utilisateur**")
        ...
        if (!mutee) return message.channel.send("**L'utilisateur n'existe pas !**");
        ...
        if (!mutee.roles.cache.has(muterole.id)) return message.channel.send("**Utilisateur non mute !**")
        try {
        mutee.roles.remove(muterole.id).then(() => {
            mutee.send(`**Bonjour, tu as été unmute sur ${message.guild.name} pour ${reason || "Aucune raison"}**`).catch(() => null)
